...
#ifndef __BT_GATT_DFU_SMP_C_H__
#define __BT_GATT_DFU_SMP_C_H__
...
#define DFU_SMP_UUID_SERVICE                                                   \
	BT_UUID_DECLARE_128(0x84, 0xaa, 0x60, 0x74, 0x52, 0x8a, 0x8b, 0x86,    \
...
#define DFU_SMP_UUID_CHAR                                                      \
	BT_UUID_DECLARE_128(0x48, 0x7c, 0x99, 0x74, 0x11, 0x26, 0x9e, 0xae,    \
 ...
 */
typedef void (*bt_gatt_dfu_smp_rsp_part_cb)(struct bt_gatt_dfu_smp_c *dfu_smp_c);
 ...
 */
typedef void (*bt_gatt_dfu_smp_error_cb)(struct bt_gatt_dfu_smp_c *dfu_smp_c,
					 int err);
...
struct bt_gatt_dfu_smp_c {
   ...
   */
	...
	/* file size for fs downloads */
	uint32_t file_size;

	/* running count of total bytes downloaded so far */
	uint32_t downloaded_bytes;

	/* keep track of # of log entries downloaded so far */
	uint32_t entry_count;

	/* keep track of # of entry bytes downloaded so far */
	uint32_t entry_downloaded_bytes;

	/* keep track of the entry protocol version */
	uint16_t entry_protocol_version;

	/* store the entry size for the currently downloading log file */
	uint16_t entry_size;

	/* count of records parsed */
	uint16_t rec_cnt;

	/* count of entries parsed */
	uint16_t ent_cnt;

	/* store a copy of the ct_log_header for use while parsing records */
	struct ct_log_header_v2 ct_log_header;
};
