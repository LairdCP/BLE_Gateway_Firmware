...
One sensor that can be used is the BL654 Sensor Board. This sensor ships with the Pinnacle 100 development kit (DVK) and measures temperature, humidity, and pressure - alternatively the BL5340 ESS demo application can be loaded to a second BL5340 which provides the same service, this is available in the sample_apps/ess_demo folder as [seen here](https://github.com/LairdCP/Sample_Apps_Common/tree/master/ess_dem) and a pre-built image can be [downloaded from here](https://github.com/LairdCP/BL5340_Firmware_Manifest/releases).
