...
static const char * WriteableLocalObject[MAX_WRITEABLE_LOCAL_OBJECTS] = {\
			...
			BATTERY_BAD_STRING, ODR_STRING, SCALE_STRING, ACT_THRESH_STRING,\
			MAX_LOG_SIZE_STRING};
...
static bool (*LocalConfigUpdate[MAX_WRITEABLE_LOCAL_OBJECTS])(int) = {\
			...
			UpdateBatteryBadThreshold, UpdateOdr, UpdateScale,\
			UpdateActivityThreshold, UpdateMaxLogSize};

static int (*LocalConfigGet[MAX_WRITEABLE_LOCAL_OBJECTS])() = {\
			GetBatteryLowThreshold, GetBatteryThreshold0,\
			GetBatteryThreshold1, GetBatteryThreshold2,\
			GetBatteryThreshold3, GetBatteryThreshold4,\
			GetBatteryBadThreshold, GetOdr, GetScale,\
			GetActivityThreshold, GetMaxLogSize};
...
static bool ValuesUpdated(u16_t Value);
...
static void BuildAndSendLocalConfigNullResponse()
{
	...
	snprintk(pMsg->topic, CONFIG_AWS_TOPIC_MAX_SIZE, "%s", awsGetGatewayUpdateDeltaTopic());
...
static void BuildAndSendLocalConfigResponse()
{
	...
	do
	{
		/* send "null" if the value was succesfully updated */
		if(ValuesUpdated(LocalConfigUpdateBits[Index]))
		{
			ShadowBuilder_AddNull(pMsg, WriteableLocalObject[Index]);
		}
	...
	do
	{
		...
		ShadowBuilder_AddUint32(pMsg, WriteableLocalObject[Index], Value);
	} while (++Index < MAX_WRITEABLE_LOCAL_OBJECTS);
	/* end reported */
	ShadowBuilder_EndGroup(pMsg);
	/* end state */
...
static int ParseLocalConfig(const char *pJson, const char *ObjectName)
{
	...
	if (ObjectLocation > 0) {
		 ...
		 * this is a valid assumtion for the current schema.
		 */
		ObjectData = ConvertInt(pJson, ObjectLocation);
	}
	else {
	...
	if (!getAcceptedTopic)
	{
		...
		*/
		...
		if (FindVersion(pJson, &version) && (pState != NULL)) {
			local_updates = 0;
			do {
				ObjectData = ParseLocalConfig(pJson, WriteableLocalObject[ObjectIndex]);
				if (ObjectData > 0) {
					/* flag values that have update requests */
					local_updates |= LocalConfigUpdateBits[ObjectIndex];
					/* execute the local updates */
					ValueUpdated = (*LocalConfigUpdate[ObjectIndex])(ObjectData);
...
static bool FindVersion(const char *pJson, u32_t *pVersion)
{
	jsonIndex = 1;
	int versionLocation =
		FindType(pJson, "version", JSMN_PRIMITIVE, 0);
	if (versionLocation > 0) {
		versionLocation += 1; /* index of data */
		*pVersion = ConvertInt(pJson, versionLocation);
		return true;
