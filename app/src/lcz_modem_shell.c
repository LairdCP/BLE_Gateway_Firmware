...
/******************************************************************************/
static K_DELAYED_WORK_DEFINE(log_work, log_restore_handler);
			     ...
			     char **argv)
{
	...
	if ((argc == 3) && (argv[1] != NULL) && (argv[2] != NULL)) {
		...
		k_delayed_work_submit(&log_work, K_SECONDS(log_restore_delay));
	} else {
		shell_error(shell,
			    "Invalid parameter"
...
static void log_restore_handler(struct k_work *work)
{
	...
	mdm_hl7800_log_filter_set(CONFIG_MODEM_LOG_LEVEL);
}
